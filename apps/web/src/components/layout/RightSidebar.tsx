...
type Topic = { id: string; tag: string; posts: number; category: string };
type Creator = { id: string; username: string; full_name?: string; avatar_url?: string; followers_count: number };

const proverb = {
  kikuyu: 'Mũndũ nĩ andũ.',
  english: 'A person is people — we become ourselves through community.'
};
...
export default function RightSidebar() {
  ...
  useEffect(() => {
    void Promise.allSettled([
      axiosInstance.get('/posts/hashtags/trending?limit=5').then(({ data }) => {
        const hashtags = data.data?.hashtags || [];
        setTopics(hashtags.map((item: { name: string; posts_count: number }) => ({
          id: item.name,
          tag: `#${item.name}`,
      ...
      axiosInstance.get('/users/suggestions?limit=4').then(({ data }) => {
  ...
  const follow = async (creatorId: string) => {
    await axiosInstance.post(`/follows/${creatorId}/follow`);
  ...
  return (
    <aside className="no-scrollbar sticky top-[68px] hidden h-[calc(100vh-68px)] w-[340px] shrink-0 space-y-4 overflow-y-auto py-4 xl:block">
      <section className="overflow-hidden rounded-[26px] bg-[#071a15] p-5 text-white shadow-[0_18px_50px_rgba(7,26,21,.16)]">
        ...
        <blockquote className="font-heading text-[21px] font-semibold leading-snug">“{proverb.kikuyu}”</blockquote><p className="mt-3 text-xs leading-relaxed text-white/50">{proverb.english}</p><div className="mt-5 h-1 w-16 rounded-full bg-gradient-to-r from-[#e55d3d] to-[#f3c969]" />
      </section>
      ...
      <section className="rounded-[26px] border border-border/70 bg-card p-5 shadow-sm">
        ...
        {creators.length === 0 ? <p className="py-4 text-xs text-muted-foreground">You are caught up with community suggestions.</p> : <div className="space-y-3">{creators.map((creator) => (<div key={creator.id} className="flex items-center gap-3">{creator.avatar_url ? <img src={creator.avatar_url} alt={creator.full_name || creator.username} className="h-11 w-11 rounded-2xl object-cover" /> : <span className="grid h-11 w-11 place-items-center rounded-2xl bg-muted font-bold">{creator.username.slice(0, 1).toUpperCase()}</span>}<div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{creator.full_name || creator.username}</p><p className="text-[10px] text-muted-foreground">@{creator.username} · {formatNumber(creator.followers_count)} people</p></div><button onClick={() => void follow(creator.id)} aria-label={`Follow ${creator.username}`} className="grid h-9 w-9 place-items-center rounded-xl bg-[#f3c969]/25 text-[#8b5c08] transition hover:bg-[#f3c969]"><UserPlus className="h-4 w-4" /></button></div>))}</div>}
      </section>
