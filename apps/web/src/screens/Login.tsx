...
const Login = () => {
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ec] text-[#092019] lg:grid lg:grid-cols-2">
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-7 sm:px-8 lg:py-8">
        <div className="w-full max-w-sm">
          <div className="mb-5 flex items-center gap-3 animate-fade-up" style={{ animationDelay: '0.05s' }}>
            <div className="relative shrink-0">
              <div className="absolute inset-0 rounded-full bg-gradient-to-r from-[#28624c] to-[#e55d3d] blur-xl opacity-25 animate-pulse" />
              <img src={doveLogo} alt="Thutha" className="relative h-14 w-14 animate-float object-contain" />
            </div>
            <div>
              <h1 className="font-heading text-2xl font-bold tracking-tight text-[#092019]">Thutha</h1>
              <p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#9a6b12]">Tũgĩe hamwe</p>
            </div>
          </div>
          <div className="mb-5 animate-fade-up"><p className="mb-2 text-sm font-bold uppercase tracking-[.18em] text-[#c45b3e]">{mode === 'forgot' ? 'Account recovery' : mode === 'register' ? 'Join the circle' : 'Welcome home'}</p><h2 className="font-heading text-3xl font-bold tracking-tight text-[#092019]">{mode === 'forgot' ? 'Reset your password' : mode === 'register' ? 'Create your Thutha' : 'Come on in.'}</h2><p className="mt-3 text-sm leading-6 text-[#5f6e67]">{mode === 'forgot' ? 'Enter your email and we’ll send a secure reset link.' : 'A modern social home for Kikuyu stories, people and possibilities.'}</p></div>
          {error && <div className="mb-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
          {mode === 'forgot' ? <form onSubmit={handleForgot} className="space-y-3"><label className="block text-sm font-semibold">Email address<input value={email} onChange={e => setEmail(e.target.value)} type="email" required placeholder="you@example.com" className="mt-2 w-full rounded-2xl border border-[#d9ded7] bg-white px-4 py-3 outline-none transition focus:border-[#c45b3e] focus:ring-4 focus:ring-[#c45b3e]/10" /></label><button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#e55d3d] px-4 py-3 font-bold text-white shadow-lg shadow-[#e55d3d]/20 disabled:opacity-50">{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <Mail className="h-5 w-5" />} Send reset link</button><button type="button" onClick={() => { setMode('login'); setError(''); }} className="w-full text-sm font-bold text-[#28624c]">Back to login</button></form> : <>
            <button onClick={handleGoogleLogin} disabled={busy} className="flex w-full animate-fade-up items-center justify-center gap-3 rounded-2xl border border-[#d9ded7] bg-white px-4 py-3 font-bold text-[#25352e] shadow-sm transition hover:-translate-y-0.5 hover:shadow-md disabled:opacity-50">{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <span className="grid h-5 w-5 place-items-center rounded-full text-sm font-black text-[#4285F4]">G</span>} Continue with Google</button>
            <div className="my-4 flex items-center gap-3 text-xs font-semibold text-[#91a097]"><span className="h-px flex-1 bg-[#dfe4dc]" />or use email<span className="h-px flex-1 bg-[#dfe4dc]" /></div>
            <form onSubmit={submit} className="space-y-3 animate-fade-up" style={{ animationDelay: '0.2s' }}>
              {mode === 'register' && <label className="block text-sm font-semibold">Full name<input value={fullName} onChange={e => setFullName(e.target.value)} required placeholder="Your full name" className="mt-2 w-full rounded-2xl border border-[#d9ded7] bg-white px-4 py-3 outline-none focus:border-[#c45b3e] focus:ring-4 focus:ring-[#c45b3e]/10" /></label>}
              <label className="block text-sm font-semibold">Email address<input value={email} onChange={e => setEmail(e.target.value)} type="email" required placeholder="you@example.com" className="mt-2 w-full rounded-2xl border border-[#d9ded7] bg-white px-4 py-3 outline-none focus:border-[#c45b3e] focus:ring-4 focus:ring-[#c45b3e]/10" /></label>
              <label className="block text-sm font-semibold">Password<span className="relative mt-2 block"><input value={password} onChange={e => setPassword(e.target.value)} type={showPassword ? 'text' : 'password'} required minLength={8} placeholder="At least 8 characters" className="w-full rounded-2xl border border-[#d9ded7] bg-white px-4 py-3 pr-12 outline-none focus:border-[#c45b3e] focus:ring-4 focus:ring-[#c45b3e]/10" /><button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#718078]" aria-label="Toggle password visibility">{showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></span></label>
              {mode === 'register' && <label className="block text-sm font-semibold">Confirm password<input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} type={showPassword ? 'text' : 'password'} required minLength={8} placeholder="Repeat your password" className="mt-2 w-full rounded-2xl border border-[#d9ded7] bg-white px-4 py-3 outline-none focus:border-[#c45b3e] focus:ring-4 focus:ring-[#c45b3e]/10" /></label>}
              <button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#092019] px-4 py-3 font-bold text-white shadow-lg shadow-[#092019]/15 transition hover:-translate-y-0.5 disabled:opacity-50">{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <ArrowRight className="h-5 w-5" />} {mode === 'register' ? 'Create account' : 'Log in'}</button>
            </form>
            <div className="mt-4 flex items-center justify-between text-sm"><button type="button" onClick={() => { setMode(mode === 'register' ? 'login' : 'register'); setError(''); }} className="font-bold text-[#28624c]">{mode === 'register' ? 'Already have an account? Log in' : 'Create an account'}</button>{mode === 'login' && <button type="button" onClick={() => { setMode('forgot'); setError(''); }} className="font-semibold text-[#718078] hover:text-[#c45b3e]">Forgot password?</button>}</div>
          </>}
          <div className="mt-5 flex items-start gap-3 rounded-2xl border border-[#e5e8e1] bg-white/60 p-4 text-xs leading-5 text-[#718078]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#28624c]" /><span>Your account is protected with secure authentication and encrypted sessions.</span><Check className="ml-auto mt-0.5 h-4 w-4 shrink-0 text-[#28624c]" /></div>
          <p className="mt-4 text-center text-xs text-[#91a097]">By continuing, you agree to our <span className="font-semibold text-[#28624c]">Terms of Service</span> and <span className="font-semibold text-[#28624c]">Privacy Policy</span>.</p>
        </div>
      </section>
    </main>
  );
